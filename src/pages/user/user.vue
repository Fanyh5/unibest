  ...
  <view>
    <view class="header">
      <up-image
        ...
        round
        :enable-preview="true"
        src="https://pic2.zhimg.com/v2-1a79b84f5d33098805484ba1ec56456d_1440w.jpg"
      >
        <template #error>
      ...
      <view class="mt-16 ml-5 text-xl">
        <text v-if="isLogined">微信用户</text>
        <text v-else @click="navigateToLogin()">登录/注册</text>
      </view>
    </view>
    <view class="pointBox -translate-y-6 py-4">
      <!-- 用户资产信息 -->
      <view class="grid grid-cols-3 text-lg">
        <view class="text-center" @click="navigateTo('/pages/wallet/index')">
          <view class="text-gray-600">预存款</view>
          <view class="text-[#ff0000] font-medium">¥12.54</view>
        </view>
        <view class="text-center" @click="navigateTo('/pages/cart/coupon/myCoupon')">
          <view class="text-gray-600">优惠券</view>
          <view class="text-[#ff0000] font-medium">8</view>
        </view>
        <view class="text-center" @click="navigateTo('/pages/mine/myTracks')">
      ...
      <view class="grid grid-cols-5 gap-2 justify-items-center text-center mt-4">
        ...
        <view class="order-item" @click="navigateTo('/pages/order/list?status=3')">
